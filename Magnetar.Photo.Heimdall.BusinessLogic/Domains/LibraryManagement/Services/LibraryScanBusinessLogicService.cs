...
public sealed class LibraryScanBusinessLogicService(ILibraryCatalogDataAccessService catalog)
{
    ...
    public async Task<ScanResultBusinessLogicModel> ScanAsync(LibraryRootDataAccessModel library, CancellationToken cancellationToken = default)
    {
        var count = await catalog.ScanLibraryAsync(library, cancellationToken);
        return ScanResultBusinessLogicMapper.FromCatalogueCount(library.Id, count);
    }
}
