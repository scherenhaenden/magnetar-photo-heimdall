...
{
    try { action(); }
    catch (Exception ex) when (ex is RemoteContractsValidationException or ArgumentException) { return; }
    throw new InvalidOperationException($"Expected validation failure for: {description}");
}

static ThermalSnapshotRemoteContractsModel AvailableSnapshot(decimal celsius, DateTimeOffset observedAt)
    => new(TelemetryAvailabilityRemoteContractsModel.Available, observedAt,
        [new ThermalReadingRemoteContractsModel("cpu0", celsius, observedAt, 1.0m, ThermalStateRemoteContractsModel.Normal)]);

static ThermalSnapshotRemoteContractsModel UnavailableSnapshot(string reason = "no sensor")
    => new(TelemetryAvailabilityRemoteContractsModel.Unavailable, DateTimeOffset.UtcNow, [], reason);
...
var policy = new WorkloadPolicyRemoteContractsModel(
    WarningCelsius: 70m,
    HighCelsius: 80m,
    CriticalCelsius: 90m,
...
var controller = new ThermalWorkloadRemoteContractsService(policy);

// ---- 1. Normal reading ----
var d1 = controller.Observe(AvailableSnapshot(65m, t0), t0);
Verify(d1.State == ThermalStateRemoteContractsModel.Normal, "65 C must be Normal");
...
Verify(d2.State == ThermalStateRemoteContractsModel.Warning, "72 C must be Warning");
...
Verify(d3.State == ThermalStateRemoteContractsModel.High, "82 C must be High");
...
Verify(d4.State == ThermalStateRemoteContractsModel.Critical, "91 C must be Critical");
...
Verify(d5.State == ThermalStateRemoteContractsModel.Critical, "Temperature 60 C at 1 min must still be blocked (Cooling, reported as Critical)");
...
Verify(controller.CurrentState == ThermalStateRemoteContractsModel.Critical, "CurrentState must be Critical during cooling");
...
Verify(d6.State == ThermalStateRemoteContractsModel.Critical, "Temperature 67 C (> hysteresis target 65) must still be blocked");
...
Verify(stillCooling.State == ThermalStateRemoteContractsModel.Critical, "The continuous safe interval begins after the unsafe reading");
var tResume = tSafeStart.AddMinutes(2);
var d7 = controller.Observe(AvailableSnapshot(63m, tResume), tResume);
Verify(d7.State == ThermalStateRemoteContractsModel.Normal, "63 C after full cooling must be Normal");
...
Verify(d8.State == ThermalStateRemoteContractsModel.Unavailable, "Explicit Unavailable snapshot must set Unavailable state");
...
var staleSnapshot = new ThermalSnapshotRemoteContractsModel(
    TelemetryAvailabilityRemoteContractsModel.Available,
    freshNow,
    [new ThermalReadingRemoteContractsModel("cpu0", 65m, staleObservedAt, 1.0m, ThermalStateRemoteContractsModel.Normal)]);
var d9 = controller.Observe(staleSnapshot, freshNow);
Verify(d9.State == ThermalStateRemoteContractsModel.Unavailable, "Stale reading must be degraded to Unavailable");
...
var recoveryController = new ThermalWorkloadRemoteContractsService(policy);
var recoveryCritical = recoveryController.Observe(AvailableSnapshot(91m, t0), t0);
Verify(recoveryCritical.State == ThermalStateRemoteContractsModel.Critical, "Recovery setup must enter Critical");
var recoveryOutage = recoveryController.Observe(UnavailableSnapshot("sensor disconnected"), t0.AddSeconds(10));
Verify(recoveryOutage.State == ThermalStateRemoteContractsModel.Critical && recoveryOutage.EffectiveConcurrency == 0,
...
Verify(recoverySafe.State == ThermalStateRemoteContractsModel.Critical, "Recovered telemetry must restart the continuous safe interval");
var recoveryResumed = recoveryController.Observe(AvailableSnapshot(64m, t0.AddMinutes(3)), t0.AddMinutes(3));
Verify(recoveryResumed.State == ThermalStateRemoteContractsModel.Normal, "Recovery may resume only after the new safe interval");
Console.WriteLine("[10] Cooling gate survives telemetry outage.");

// ---- 11. UnavailableThermalRemoteContractsService — never fabricates Celsius ----
var unavailableProvider = new UnavailableThermalRemoteContractsService("No SMC access on this runner.");
var providerSnapshot = unavailableProvider.GetSnapshot();
Verify(providerSnapshot.Availability == TelemetryAvailabilityRemoteContractsModel.Unavailable, "Provider snapshot must be Unavailable");
...
Console.WriteLine($"[11] UnavailableThermalRemoteContractsService: Availability={providerSnapshot.Availability}, Reason='{providerSnapshot.Reason}'");

// ---- 12. OperationRequestRemoteContractsModel — valid path accepted ----
var caps = new AgentCapabilitiesRemoteContractsModel(
    ProtocolVersionRemoteContractsModel.V1,
    [new AgentRootRemoteContractsModel("photos", "Photos Library")],
    [],
    new HashSet<OperationKindRemoteContractsModel> { OperationKindRemoteContractsModel.Hash, OperationKindRemoteContractsModel.Scan },
    8,
    true);

var validOp = new OperationRequestRemoteContractsModel(
    ProtocolVersionRemoteContractsModel.V1, "photos", "2026/trip/DSC0001.nef", OperationKindRemoteContractsModel.Hash, 2, "idem-key-abc");
validOp.ValidateAgainst(caps);
Console.WriteLine("[12] Valid OperationRequestRemoteContractsModel accepted.");
...
})
{
    ExpectContractViolation(
        () => new OperationRequestRemoteContractsModel(ProtocolVersionRemoteContractsModel.V1, "photos", badPath, OperationKindRemoteContractsModel.Hash, 1, "k1")
...
{
    ExpectContractViolation(
        () => new OperationRequestRemoteContractsModel(ProtocolVersionRemoteContractsModel.V1, "photos", traversal, OperationKindRemoteContractsModel.Hash, 1, "k2")
                  .ValidateAgainst(caps),
...
ExpectContractViolation(
    () => new OperationRequestRemoteContractsModel(ProtocolVersionRemoteContractsModel.V1, "unknown-root", "image.nef", OperationKindRemoteContractsModel.Hash, 1, "k3")
              .ValidateAgainst(caps),
...
ExpectContractViolation(
    () => new OperationRequestRemoteContractsModel(new ProtocolVersionRemoteContractsModel(2, 0), "photos", "image.nef", OperationKindRemoteContractsModel.Hash, 1, "k4")
              .ValidateAgainst(caps),
...
ExpectContractViolation(
    () => new OperationRequestRemoteContractsModel(ProtocolVersionRemoteContractsModel.V1, "photos", "image.nef", OperationKindRemoteContractsModel.Hash, 1, "")
              .ValidateAgainst(caps),
...
ExpectContractViolation(
    () => new OperationRequestRemoteContractsModel(ProtocolVersionRemoteContractsModel.V1, "photos", "image.nef", OperationKindRemoteContractsModel.Hash, 0, "k5")
              .ValidateAgainst(caps),
    "Concurrency=0");
ExpectContractViolation(
    () => new OperationRequestRemoteContractsModel(ProtocolVersionRemoteContractsModel.V1, "photos", "image.nef", OperationKindRemoteContractsModel.Hash, 99, "k6")
              .ValidateAgainst(caps),
    "Concurrency exceeds MaximumConcurrency");
Console.WriteLine("[18] Out-of-range concurrency correctly rejected.");

// ---- 19. WorkloadPolicyRemoteContractsModel validation catches bad configuration ----
ExpectContractViolation(
    () => new WorkloadPolicyRemoteContractsModel(90m, 80m, 70m, 5m, TimeSpan.FromMinutes(2), TimeSpan.FromSeconds(30), 8, 4, 1).Validate(),
    "Inverted thresholds");
ExpectContractViolation(
    () => new WorkloadPolicyRemoteContractsModel(70m, 80m, 90m, 5m, TimeSpan.FromMinutes(2), TimeSpan.FromSeconds(30), 1, 4, 1).Validate(),
    "WarningConcurrency > NormalConcurrency");
ExpectContractViolation(
    () => new WorkloadPolicyRemoteContractsModel(70m, 80m, 90m, 70m, TimeSpan.FromMinutes(2), TimeSpan.FromSeconds(30), 8, 4, 1).Validate(),
    "Hysteresis cannot make the safe threshold unreachable");
Console.WriteLine("[19] WorkloadPolicyRemoteContractsModel validation catches bad configuration.");

// ---- 20. AgentRootRemoteContractsModel id validation ----
ExpectContractViolation(() => new AgentRootRemoteContractsModel("bad path/root!", "Bad").Validate(), "Root id with invalid chars");
ExpectContractViolation(() => new AgentRootRemoteContractsModel("", "Empty").Validate(), "Empty root id");
Console.WriteLine("[20] AgentRootRemoteContractsModel id validation works.");
